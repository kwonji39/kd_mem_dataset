...
class Pipeline:
    ...
    def train(
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for x_train, y_train, idx_batch in pbar:
                ...
                valid_indices_mask = np.zeros_like(idx_batch, dtype=bool)
                for i, idx in enumerate(idx_batch):
                    if idx < len(samples_bitvector):
                        valid_indices_mask[i] = True
                x = x[valid_indices_mask]
                y_truth = y_truth[valid_indices_mask]
                idx_batch = idx_batch[valid_indices_mask]

                # s1_selector = samples_bitvector[idx] == 1
                # x = x[s1_selector]
                # y_truth = y_truth[s1_selector]
                # idx = idx[s1_selector]
                s1_selector = samples_bitvector[idx_batch] == 1
                ...
                for i in range(len(idx_batch)):
                    ...
                    # print(grad)
                    grad_log1[idx_batch[i]][epoch-1] = torch.mean(torch.abs(grad[0]))
                    grad_log2[idx_batch[i]][epoch-1] = torch.mean(torch.abs(grad[1]))
            ...
            if epoch == 1 or epoch % validation_score_epoch == 0:
                ...
                for test_name, test_loader in self.test_loaders.items():
                    ...
                    self.model.eval()
                    with torch.no_grad():
                        for x_test, y_test, _ in tqdm(self.test_loaders[test_name], desc="Validation '{}' epoch {}".format(test_name, epoch)):
