...
if __name__ == "__main__":
    ...
    for i in range(len(mem_thresh)):
        for j in range(1, disjoint_sets+1):
            ...
            test_name = "train_kd_res50_0-{}_1-conv2_disjoint_sets_0-{}_{}".format(mem1, mem1, j)

            preprocessor = Preprocessor((32,32))
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

            # model = ResNet.resnet18(num_classes=100, include_top=True)
            model = ConvNet.convnet2(num_classes=100, include_top=True)
            model_path = input()
            ...
            test_set = LabeledPickleDatasetMapper(
                ...
                None,
                # "../../../dataset/scratch_fullsets/subset_0-1.csv",
